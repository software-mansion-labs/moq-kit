...
- **iOS publisher states:** `Idle / Publishing / Stopped / Error(message)`. Session teardown stops its publishers. Audio/video return `PublishedMediaTrack` with requested `isEnabled` and async `setEnabled(bool)`; states are `Idle / Disabled / Starting / Active / Failed(message) / Stopped`. Raw data returns `PublishedTrack` with state and terminal `stop()` only. Publisher, track, and capture cleanup awaits teardown. Empty publishers stay open until explicit stop or session end.
- **Android publisher states:** `Idle / Publishing / Stopped / Error(message)` — but Android never actually enters `Error`, and it does not react to the session dying either: an Android publisher sits in `Publishing` indefinitely after its session is gone, so the session's own state is the only signal that the broadcast stopped. Wire failure UI to **events**: `TrackStarted` / `TrackStopped` / a per-track error (`TrackError(name, message)`); per-track `PublishedTrack.state` (`Idle / Starting / Active / Stopped`) with `PublishedTrack.stop()` to end one track. `session.unpublish(path)` calls `publisher.stop()`.
...
8. **iOS `Player` is `@MainActor`** — construct and drive it on the main actor. iOS capture and publication async controls move blocking setup/teardown off the main thread; await their completion.
...
| iOS: publication still active during teardown | Await `Publisher.stop()` / `PublishedTrack.stop()`; canceling UI observation alone does not stop media |
