...
- **Publishing:** create capture sources (`CameraCapture`, `MicrophoneCapture`, `MultiCameraCapture`, `ScreenCapture`) (start them explicitly when hardware is wanted) → `Publisher()` + `addVideoTrack`/`addAudioTrack`/`addDataTrack` → `session.publish(path, publisher)` → `publisher.start()`. A publisher is single-use; add all tracks before `start()`.
...
try publisher.addVideoTrack(name: "camera", source: camera, config: VideoEncoderConfig())
...
- **Publisher states:** `Idle / Publishing / Stopped / Error(message)`. Session teardown stops its publishers. Audio/video return `PublishedMediaTrack` with requested `isEnabled` and async/suspend `setEnabled(bool)`; states are `Idle / Disabled / Starting / Active / Failed(message) / Stopped`. Raw data returns `PublishedTrack` with state and terminal `stop()` only. Publisher, track, and capture cleanup awaits teardown. Empty publishers stay open until explicit stop or session end.
...
3. **`publisher.start()` does not start capture.** Add tracks → `session.publish(path, publisher)` → `publisher.start()`. Camera/microphone may be started before or after registration. Capture stop suspends enabled publication and releases hardware; track `setEnabled(false)` releases only encoding/publication. Capture start resumes enabled tracks.
...
6. **Permissions and audio sessions are the app's job.** iOS: `NSCameraUsageDescription`, `NSMicrophoneUsageDescription`, `NSLocalNetworkUsageDescription` (local relays), and configure `AVAudioSession` yourself (`.playAndRecord` to publish, `.playback` to watch) — `MicrophoneCapture` will not. Android: declare + request `CAMERA`/`RECORD_AUDIO` (`MicrophoneCapture.start()` throws on startup failure), `INTERNET`, and the MediaProjection foreground-service setup for screen capture.
...
8. **iOS `Player` is `@MainActor`** — construct and drive it on the main actor. Capture and publication async controls move blocking setup/teardown off the main thread; await their completion.
...
| No microphone audio | iOS: `AVAudioSession` isn't on a record-capable category. Android: check `RECORD_AUDIO` and the error thrown by `MicrophoneCapture.start()` |
...
| Publication still active during teardown | Await `Publisher.stop()` / `PublishedTrack.stop()`; canceling UI observation alone does not stop media |
