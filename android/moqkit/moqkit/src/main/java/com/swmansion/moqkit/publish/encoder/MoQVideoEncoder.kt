...
private const val TAG = "MoQVideoEncoder"
...
class MoQVideoEncoder(val config: MoQVideoEncoderConfig) {
    ...
    fun start(onEncodedFrame: (EncodedVideoFrame) -> Unit) {
        ...
        val mimeType = when (config.codec) {
            MoQVideoCodec.H264 -> MediaFormat.MIMETYPE_VIDEO_AVC
            MoQVideoCodec.H265 -> MediaFormat.MIMETYPE_VIDEO_HEVC
        }
    ...
    private fun handleEncodedData(raw: ByteArray, presentationUs: Long, isKeyframe: Boolean) {
        ...
        val payload = when (config.codec) {
            MoQVideoCodec.H264 -> AvccConverter.annexBToAvcc(raw)
            MoQVideoCodec.H265 -> raw  // pass Annex-B through
        }
